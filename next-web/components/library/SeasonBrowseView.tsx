...
import type { AiringSeason } from "@/lib/season";
import AnimeCard from "./AnimeCard";
import BrowsePager from "./BrowsePager";

type StreamState = "connecting" | "streaming" | "done" | "error" | "closed";

/** Abort the HTTP fallback rather than hanging on a starved backend for minutes. */
...
export default function SeasonBrowseView({
  year,
  season,
  limit = 24,
  offset = 0,
  ...
  const runHttpFallback = useCallback(
    async (
      ...
      try {
        const response = await api.browseSeason(year, season, limit, offset, {
        ...
        emit("error", "Season browse failed");
  ...
  useEffect(() => {
    ...
    const streamUrl =
      `${backendPath("/ui/library/season/stream")}` +
  ...
  }, [year, season, limit, offset, fetchLimit, runHttpFallback]);

  const nextUrl = hasNext ? nextUrlProp : null;

  return (
    <>
      <section className="grid" data-season-stream data-season-year={year} data-season-name={season}>
        {items.map((item) => (
      ...
      <p className="page-head__subtitle" data-season-stream-empty hidden={!emptyVisible}>
