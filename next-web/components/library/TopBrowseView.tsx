...
import type { TopCategory } from "@/lib/top";
...
export default function TopBrowseView({
  category,
  limit = 24,
  offset = 0,
  prevUrl = null,
  ...
  const runHttpFallback = useCallback(
    async (
      ...
      try {
        const response = await api.browseTop(category, limit, offset);
  ...
  useEffect(() => {
    ...
    const streamUrl =
      `${backendPath("/ui/library/top/stream")}` +
  ...
  }, [category, limit, offset, fetchLimit, runHttpFallback]);

  const nextUrl = hasNext ? nextUrlProp : null;

  return (
    <>
      <section className="grid" data-top-stream data-top-category={category}>
        {items.map((item) => (
      ...
      <p className="page-head__subtitle" data-top-stream-empty hidden={!emptyVisible}>
