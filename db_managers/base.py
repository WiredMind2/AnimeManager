...
class BaseDB:
    ...
    def pooled_connection(self):
        ...
        if not self.USE_CONNECTION_POOL:
            # Fallback to regular connection management
            yield self
            return
        ...
        try:
            ...
            # Temporarily replace self.db and self.cur with pooled connection
            original_db = getattr(self, 'db', None)
            original_cur = getattr(self, 'cur', None)

            self.db = conn
            self.cur = None  # Will be created when needed
