...
try:
    ...
    from classes import Anime, AnimeList, Character, NoneDict
    from constants import Constants
    from db_managers.base import BaseDB, ConnectionPool
...
class EmbeddedMariaDB(BaseDB):
    ...
    def __init__(self, settings=None) -> None:
        ...
        # Attempt to handle common termination signals so we stop the child process cleanly
        try:
            ...
            def _sig_handler(signum, frame):
                ...
                try:
                    ...
                    pass
                # Re-raise KeyboardInterrupt for interactive sessions
...
"""
    ...
    def _start_mariadb_server(self):
        ...
        try:
            # Cross-platform: on POSIX start new session so we can signal the process group if needed
            ...
            if os.name == "nt" and _WinJob is not None:
                try:
                    # Create a module-level job and add process handle
                    if not getattr(self, "_job", None):
                        self._job = _WinJob()
                    # Assign the child process to the job using PID (avoid private attributes)
                    if getattr(self.process, "pid", None):
                        self._job.add(self.process.pid)
                    else:
                        # Fallback: try to access private handle if pid is not yet set
                        try:
                            self._job.add(self.process._handle)
                        except Exception:
                            pass
                except Exception as e:
                    # Log but continue; job object is best-effort
                    self.log(
                        "DB_ERROR",
            ...
            if self.process.poll() is not None:
                ...
                # Timeout waiting for server start - attempt to stop and raise
                self.stop_server()
    ...
    def insert(self, data, table, save=True):
        ...
        with self:
            self.sql(sql, values, save=False)
    ...
    def update(self, id, data, table, save=True):
        ...
        if args:
            # SQL injection fix: validate table name
            validated_table = self._validate_table_name(table)
