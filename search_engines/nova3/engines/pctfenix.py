...
class pctfenix(object):
    ...
    class HTMLParser2(HTMLParser):
        def handle_starttag(self, tag, attrs):
            if tag == "div":
                ...
                torrent_num = torrent_num[
                    torrent_num.find("(") + 1 : torrent_num.find(")")
                ]
    ...
    def modCap(self, id, title):
        s = {"id": id}
        html = pctfenix.do_post(
            pctfenix, pctfenix.url + "/controllers/show.chapters.php", s
        )
    ...
    def loadDD(self, CNAME, CID, O):
        ...
        html = pctfenix.do_post(
            pctfenix, pctfenix.url + "/controllers/load.chapters.type.php", s
        )
