...
class limetorrents(object):
    ...
    class MyHtmlParser(HTMLParser):
        ...
        def handle_starttag(self, tag, attrs):
            ...
            if (
                ...
                and self.findTable
                and (
                    params.get("bgcolor") == "#F4F4F4"
                    or params.get("bgcolor") == "#FFFFFF"
                )
            ):  # noqa
                self.inside_tr = True
                self.current_item = {}
        ...
        def handle_data(self, data):
            if self.inside_tr and self.item_name:
                if self.item_name == "size" and (
                    data.endswith("MB") or data.endswith("GB")
                ):
    ...
    def search(self, query, cat="all"):
        ...
        page = 1
        while True:
            page_url = "{0}/search/{1}/{2}/seeds/{3}/".format(
                self.url, category, query, page
            )
