...
class bt4g(object):
    ...
    class MyHTMLParser(HTMLParser):
        ...
        def handle_starttag(self, tag, attrs):
            ...
            if tag == "div":
                if (
                    not self.is_in_container
                    and attr_dict.get("class", "") == "container"
                ):
                    self.is_in_container = True
            elif tag == "a":
                if self.is_in_container and all(
                    x in attr_dict for x in ["title", "href"]
                ):
    ...
    def download_torrent(self, info):
        if "trackerlist" not in self.__dict__:
            self.trackerlist = json.loads(
                retrieve_url("https://downloadtorrentfile.com/trackerlist")
            )
        magnet = f"magnet:?xt=urn:btih:{info.split('/')[-1]}&tr=" + "&tr=".join(
            self.trackerlist
        )
        print(magnet + " " + info)
