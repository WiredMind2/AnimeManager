                ...
                if tag == "a":
                    ...
                    if "href" in attributes:
                        if attributes["href"].startswith("/hash"):
                            ...
                            self.torrent_info["desc_link"] = (
                                f"{self.engine_url}{torrent_link}"
                            )
                            magnet_id = torrent_link.split("hash/")[1].split(".html")[0]
                            self.torrent_info["link"] = (
                                f"magnet:?xt=urn:btih:{magnet_id}"
                            )
                            self.find_link = False
                            self.find_info = True
            elif self.find_info:
        ...
        def handle_data(self, data):
            if self.parse_results_per_page:
                ...
                try:
                    size, unit = [
                        x.strip()
                        for x in data.split("Size：")[1].split("Created")[0].split(" ")
                    ]
                except:
                    try:
                        size, unit = [
                            x.strip()
                            for x in data.split("ファイルサイズ：")[1]
                            .split("創建時期")[0]
                            .split(" ")
                        ]
                    except:
