...
class yts(object):
    ...
    def search(self, keyword, cat="all"):
        ...
        if j["data"]["movie_count"] and "movies" in j["data"]:
            page_of = "{}of{}".format(
                j["data"]["page_number"],
                int(math.ceil(int(j["data"]["movie_count"]) / int(j["data"]["limit"]))),
            )
            ...
            url_path = list(
                map(lambda i: i in params and params[i] or url_params[i], url_params)
            )
            url = job.urlBuilder(
            ...
            if data_container and data_container[0]:
                ...
                for hM in data_movie:
                    ...
                    if movie_id:
                        ...
                        for torrent in movies["torrents"]:
                            res = {
                                "link": job.magnetBuilder(
                                    torrent["hash"], movies["title"]
                                ),
                                "name": "{n} ({y}) [{q}]-[{p}]-[{i}]".format(
                                    n=movies["title"],
                                    y=movies["year"],
...
class score(object):
    ...
    def magnetBuilder(self, hash, name):
        return "magnet:?xt=urn:btih:{}&{}&{}".format(
            hash,
            urlencode({"dn": name}),
            "&".join(map(lambda x: "tr=" + quote_plus(x.strip()), self.tracker)),
        )
