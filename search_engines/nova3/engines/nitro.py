...
class nitro(object):
    ...
    class HTMLParser:
        ...
        def __findTorrents(self, html):
            torrents = []
            trs = re.findall(r"<tr>.*?td.+?center\">\d\d/\d\d/\d\d\d\d.+?</tr>", html)
            for tr in trs:
    ...
    def download_torrent(self, info):
        ...
        magnet_raw = retrieve_url(
            f"https://nitro.to/download_magnet.php?id={torrent_id}"
        )
    ...
    def search(self, what, cat="all"):
        ...
        for currPage in range(0, self.max_pages):
            url = "{0}tags.php?search={1}&per=50&by=desc&order=data&page={2}&where=1".format(
                self.url, what, currPage
            )
            # Some replacements to format the html source
            html = (
                retrieve_url(url).replace("	", "").replace("\n", "").replace("\r", "")
            )
            parser.feed(html)
