...
class tokyotoshokan(object):
            ...
            if self.current_item:
                if tag == "a":
                    if params["href"].startswith("magnet"):
                        ...
                        "type" in params
                        and params["type"] == "application/x-bittorrent"
                    ):
                        self.name_found = True
                        self.current_item["name"] = ""
                    elif params["href"].startswith("details"):
                        self.current_item["desc_link"] = "".join(
                            (self.url, "/", params["href"])
                        )
    ...
    def handle_more_pages(self, last_page_url, parser, query, skip_first=False):
        ...
        additional_links = re_compile(
            "\?lastid=[0-9]+&page=[0-9]+&terms={}".format(query.replace("%20", "\\+"))
        )
        ...
        for res_link in map(
            lambda link: "".join((self.url, "/search.php", link.group(0))),
            additional_links.finditer(data),
        ):
            if skip_first:
                skip_first = False
                continue
    ...
    def search(self, query, cat="all"):
        ...
        torrent_list = re_compile('(?s)<table class="listing">(.*)</table>')
        request_url = (
            "{0}/search.php?terms={1}&type={2}&size_min=&size_max=&username=".format(
        ...
        while True:
            if page_count > (page_multiplier * 5):
                last_page_url = self.handle_more_pages(
                    last_page_url, parser, query, True
                )
                page_multiplier += 1
            else:
                break
