...
# ``shared.telemetry -> shared.config -> adapters.persistence -> classes``
# circular import. Callers reach the logger through :func:`log` below,
# which materializes the real function on first use.


def log(*args, **kwargs):  # noqa: D401 - thin late-binding shim
    """Late-bound proxy for :func:`shared.telemetry.logger.log`."""
    global log
    try:
        from shared.telemetry.logger import log as _log
        ...
        from AnimeManager.shared.telemetry.logger import log as _log  # type: ignore

    log = _log  # rebind so subsequent calls bypass this shim
...
class Torrent(Item):
    ...
    @classmethod
    def from_torrent(cls, filedata):
        meta = bencoding.bdecode(filedata)
        info = meta[b"info"]

        # BitTorrent infohash: SHA-1 of the bencoded `info` dict, hex-encoded
        # (40 chars). SHA-256/base32 is non-standard and would not match the
        # hash that BitTorrent clients use to identify the torrent.
        info_hash = hashlib.sha1(bencoding.bencode(info)).hexdigest()

        name_bytes = info.get(b"name", b"")
        name = name_bytes.decode("utf-8", errors="replace") if isinstance(
            name_bytes, (bytes, bytearray)
        ) else str(name_bytes)

        trackers: list[str] = []
        announce_list = meta.get(b"announce-list")
            ...
            for tier in announce_list:
                ...
                for entry in tier:
                    if isinstance(entry, (bytes, bytearray)):
                        url = entry.decode("utf-8", errors="replace")
                        if url and url not in trackers:
                            trackers.append(url)
        announce = meta.get(b"announce")
        if isinstance(announce, (bytes, bytearray)):
            url = announce.decode("utf-8", errors="replace")
            if url and url not in trackers:
                trackers.append(url)

        # ``length`` is an integer in single-file torrents; multi-file torrents
        # expose a ``files`` list and have no top-level ``length`` field.
        ...
        return cls(hash=info_hash, name=name, trackers=trackers, size=size)
...
class ItemList:
    ...
    def __init__(self, *sources):
        ...
        self.sources = []
        self.sourceThreads = []
        # Set membership is O(1) and avoids the previous list-based O(n^2)
        # scan that grew unbounded with result count.
        self.ids = set()
        self._unidentified_counter = 0
    ...
    def identifier_wrapper(self, func):
        """Wrap an identifier function with a safe, collision-free fallback.

        The previous implementation returned ``None`` on any exception which
        meant that every subsequent unidentifiable element was treated as a
        duplicate and silently dropped. We now generate a unique synthetic id
        for those edge cases so they remain visible to the consumer.
        """
        ...
        def wrapper(*args, **kwargs):
            ...
            if value is None:
                with self._id_lock:
                    ...
                    return ("__unidentified__", self._unidentified_counter)
            return value
    ...
    def sourceListener(self, s):
        ...
        while not self.stop:
            ...
            # 	break
                ...
                self.sources.remove(s)
                # Keep stream alive even when one source fails.
                break
            else:
...
class TorrentList(ItemList):
    ...
    Uses a deterministic fingerprint based on the magnet infohash when
    available and falls back to a tuple of stable identifying fields.
    The previous fallback (``hash(dict.values())``) was broken because
    ``dict.values()`` is unhashable, which silently collapsed distinct
    torrents into a single ``None`` bucket.
    """

    _MAGNET_INFOHASH_RE = re.compile(r"xt=urn:btih:([A-Za-z0-9]+)", re.IGNORECASE)
    ...
    def identifier(self, t, *args, **kwargs):
        """Return a deterministic, hashable identifier for ``t``.

        Preference order:
            1. Explicit infohash (``t["infohash"]`` or ``t["hash"]``)
            2. Infohash extracted from the magnet link
            3. Tuple of normalized identifying fields
        """
        if not isinstance(t, dict):
            return None

        explicit = t.get("infohash") or t.get("hash")
        if explicit:
            return ("ih", str(explicit).strip().lower())

        link = t.get("link")
        if isinstance(link, str):
            match = self._MAGNET_INFOHASH_RE.search(link)
            if match:
                return ("ih", match.group(1).lower())

        name = (t.get("name") or "").strip().casefold()
        size = t.get("size")
        engine_url = (t.get("engine_url") or "").strip().lower()
        desc_link = (t.get("desc_link") or "").strip().lower()
        return ("nf", name, str(size), engine_url, desc_link)
