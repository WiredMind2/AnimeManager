...
class MyAnimeListNetWrapper(APIUtils):
    def __init__(self, tokenPath="token.json"):
        ...
        super().__init__()
        # MAL OAuth client credentials are sourced from runtime config only.
        self.CLIENT_ID, self.CLIENT_SECRET = self._load_oauth_credentials()
    ...
    def _load_oauth_credentials(self):
        """Resolve MAL OAuth credentials via the shared secret provider."""
        ...
        if not client_id or not client_secret:
            self.log(
                "MAL",
                "MAL OAuth credentials are not configured. Set "
                "ANIMEMANAGER_MAL_CLIENT_ID and ANIMEMANAGER_MAL_CLIENT_SECRET.",
            )
        return client_id, client_secret

    def _persist_token(self, token):
        """Write a token to disk with owner-only permissions where possible."""
        if not token:
            return
        with open(self.tokenPath, "w") as fh:
            json.dump(token, fh, indent=4)
        try:
            os.chmod(self.tokenPath, stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            # Filesystems that don't support unix permissions (FAT, some Windows
            # configurations) silently ignore the chmod; we keep going.
            pass
    ...
    def getNewToken(self, threaded=False):
        if not self.CLIENT_ID or not self.CLIENT_SECRET:
            self.log("MAL", "Skipping interactive OAuth: credentials are missing.")
            return
        if threaded is False:
            threading.Thread(target=self.getNewToken, args=(True,), daemon=True).start()
            return
        ...
        response = requests.post(url, data, timeout=REQUEST_TIMEOUT_S)
    ...
    def refresh_token(self, r_token):
        if not self.CLIENT_ID or not self.CLIENT_SECRET:
            self.log("MAL", "Cannot refresh token: credentials are missing.")
            return
        url = "https://myanimelist.net/v1/oauth2/token"
        data = {
            "client_id": self.CLIENT_ID,
